...
class JSONPlugin:
    ...
    def __init__(self, sink_manager, mqtt_wrapper, settings):
    ...
    def start(self):
        logging.info("JSON Plugin ready! (Version %s)", PLUGIN_FIRMWARE_VERSION)

    # *****************************************************************

    def _send_message(self, sink_id, destination_address, source_endpoint, destination_endpoint, data):
        result = None
        logging.debug("send message to 0x%x (EP %d) with sink(%s) - data: %s",
    ...
    def _on_packet_request_received(self, client, userdata, message):
        logging.info("Wirepas packet request received on topic " + message.topic)
        ...
        try:
            ...
            try:
                ...
                logging.exception("Impossible to send wirepas message")
                reply_string = "Impossible to send wirepas message"

        except Exception as e:
            logging.exception("Impossible to process json command")
